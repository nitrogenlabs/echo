...
import type { EchoDevice, EchoModel, EchoSession } from "./state/echoStore.js";
...
class EchoServer {
  ...
  private handleApiRequest(
    ...
    if (path === "/api/devices" && method === "POST") {
      this.readBody(req)
        .then((body) => {
          const device: EchoDevice = JSON.parse(body);
    ...
    if (path === "/api/models" && method === "POST") {
      this.readBody(req)
        .then((body) => {
          const model: EchoModel = JSON.parse(body);
          registerModel(model);
    ...
    if (path === "/api/sessions" && method === "POST") {
      this.readBody(req)
        .then((body) => {
          const session: EchoSession = JSON.parse(body);
  ...
  listen(port: number = PORT): Promise<void> {
...
let serverInstance: EchoServer | null = null;

export function createServer(): EchoServer {
  if (serverInstance) {
...
export async function startServer(port: number = PORT): Promise<EchoServer> {
